![[Aelwyn.jpeg]]

```statblock
layout: Basic 5e Layout
image: "Elf_Watcher.png"
name: Aelwyn, [[Veilleurs|Veilleur]] de Rillifane
size: Medium
type: humanoïde
subtype: elfe
alignment: neutre bon
ac: 12
hp: 16
hit_dice: 3d8
speed: "30 ft."
stats: [8, 14, 12, 13, 15, 11]
saves:
  - wisdom: +4
skillsaves:
  - nature: +4
  - religion: +3
senses: "vision dans le noir 60 ft., Perception passive 12"
languages: "elfique, commun, sylvestre"
damage_resistances: ""
condition_immunities: ""
cr: 1/4
traits:
  - name: "Craintif et Entravé"
    desc: "Tant qu’Aelwyn est entravé ou menacé au corps à corps par un ennemi, il ne peut effectuer que l’action Se désengager ou Aider."
  - name: "Foi Retrouvée"
    desc: "Dès qu’Aelwyn est libéré ou qu’un allié réduit un gnoll à 0 PV, il peut agir normalement."
spellcasting:
  level: 2
  ability: wisdom
  dc: 12
  attack_bonus: +4
  spells:
    cantrips: ["druidcraft", "guidance"]
    1st_level: ["entangle", "cure wounds"]
actions:
  - name: "Bâton rituel"
    desc: "Attaque de mêlée : +4 pour toucher, allonge 5 ft., une cible. Touché : 3 (1d6) dégâts contondants."

```

## Rôle dans la campagne

Veilleur de [[Rillifane Rallathil]] qui étudiait le [[Temple de Rillifane]] avant d’être capturé par les [[Gnolls|gnolls]] de [[Varkh]]. Terrorisé mais lucide, il connaissait les glyphes et l’histoire du sanctuaire. Une fois libéré, il a aidé les héros à activer l’autel avec la [[Marque du Serment]].

Après la restauration du temple, il a immédiatement sympathisé avec [[Lafel]].
